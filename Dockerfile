...
RUN apt-get update && \
    ...
    wget --no-check-certificate https://mathias-kettner.de/support/1.2.8p5/check-mk-raw-1.2.8p5_0.xenial_amd64.deb && \
    (dpkg -i check-mk-raw-1.2.8p5_0.xenial_amd64.deb || true) && \
    apt-get install -y -f --no-install-recommends  && \
    apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/* && \
    rm -rf check-mk-raw-1.2.8p5_0.xenial_amd64.deb
